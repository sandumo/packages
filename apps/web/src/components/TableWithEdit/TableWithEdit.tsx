import { Box, Checkbox, Grid, Menu, MenuItem } from '@mui/material';
...
export default function TableWithEdit({
  rows,
  columns,
  getRowHref,
  getRowContextMenu,
  ...
  const handleClick = () => {
    setShowSecondBlock(!showSecondBlock);
  };

  return (
    <>
      <Button onClick={() => handleClick()} variant="contained">test</Button>
      <Grid container>
        <Grid item xs={showSecondBlock ? 10 : 12} gap={showSecondBlock ? 10 : 0}>
          <DataGrid
            rows={rows}
            columns={columns}
            autoHeight
